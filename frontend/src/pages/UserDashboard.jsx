    ...
    <PageWrapper title="My Trust Profile">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <h1 className="text-4xl font-black text-white tracking-tighter">Hello, {userData?.name || userData?.metadata?.name || user?.name || (user?.address ? `${user.address.slice(0,6)}...${user.address.slice(-4)}` : 'there')}</h1>
